	...
	public void render(PoseStack poseStack, IComponentRenderContext context, float partialTicks, int mouseX, int mouseY) {
		if(fusionRecipe != null) {
			...
			for(Ingredient ingredient : ingredients) {
				...
				poseStack.pushPose();
				poseStack.translate(xPos-MathHelper.floor(xPos), yPos-MathHelper.floor(yPos), 0);
				context.renderIngredient(poseStack, MathHelper.floor(xPos), MathHelper.floor(yPos), mouseX, mouseY, ingredient);
				poseStack.popPose();
